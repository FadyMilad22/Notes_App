...
@Composable
private fun HomeScreenPreview(navController: NavController = rememberNavController(),) {

    NoteListItem(note = Note(5, "Hello this is a test note", "Hi Trial") ){

        val note = Note(5, "Hello this is a test note", "Hi Trial")
        navController.navigate("$EDIT_NOTE/${note.id}/${note.noteDetails}/${note.noteTitle}")
    }
}